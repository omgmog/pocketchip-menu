...
class Wifi(Widget):
    def __init__(self, parent=None):
        ...
        if IS_LINUX:
            ...
            self.image = pygame.transform.scale(pygame.image.load(assetpath('wifi-100.png')), self.size)

    def update(self):
        if self.wifi_device is None:
            return
        if self.wifi_device.ActiveConnection is None:
            self.image = pygame.transform.scale(pygame.image.load(assetpath('wifi-disconnected.png')), self.size)
            return
        ...
        if self.wifi_device.ActiveConnection.Devices[0].ActiveAccessPoint.Strength > 50:
            self.image = pygame.transform.scale(pygame.image.load(assetpath('wifi-75.png')), self.size)
            return
        if self.wifi_device.ActiveConnection.Devices[0].ActiveAccessPoint.Strength > 25:
            self.image = pygame.transform.scale(pygame.image.load(assetpath('wifi-50.png')), self.size)
            return
        else:
            self.image = pygame.transform.scale(pygame.image.load(assetpath('wifi-25.png')), self.size)
            return